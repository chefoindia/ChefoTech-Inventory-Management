...
## Public website
- `POST /api/v1/leads` is the only unauthenticated write endpoint: zod-validated, 8 requests/hour/IP, honeypot field, consent required; leads are stored before any email is attempted and the notification goes only to `LEADS_NOTIFY_EMAIL`.
- The client bundle contains only `NEXT_PUBLIC_*` values (site URL, company contact details, analytics ids). The build output is scanned for key patterns before release.
- Analytics scripts load only when `NEXT_PUBLIC_ANALYTICS_PROVIDER` is set; events are page paths and CTA labels, never form contents.
- The signed-in application, invitations, `/share` links and the API are `noindex`/disallowed for crawlers; only marketing, login and register pages are in the sitemap.
