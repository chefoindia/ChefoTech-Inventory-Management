    ...
    <Dialog open={open} onOpenChange={(o) => !create.isPending && onOpenChange(o)}>
      <DialogContent title={purchase ? `Return to supplier against ${purchase.number}` : 'Return goods to supplier'} description="Stock leaves the selected batch; the value is priced at the batch purchase cost." size="xl">
        <div className="space-y-4">
          {!fixedSupplier && !purchase ? <FormField label="Supplier" htmlFor="pr-supplier" required><SupplierPicker value={supplierId} onChange={setSupplierId} /></FormField> : null}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <FormField label="Settlement" htmlFor="pr-settle"><Select value={settlement} onChange={(e) => setSettlement(e.target.value as typeof settlement)}><option value="credit_note">Credit note (reduce payable)</option><option value="refund">Cash refund received</option></Select></FormField>
            {settlement === 'refund' ? <><FormField label="Refund method" htmlFor="pr-method"><Select value={method} onChange={(e) => setMethod(e.target.value as PaymentMethod)}>{TENDER_METHODS.map((m) => <option key={m} value={m}>{PAYMENT_METHOD_LABELS[m]}</option>)}</Select></FormField><FormField label="Refund amount" htmlFor="pr-amt" hint="Leave blank to use the return value."><Input type="number" min={0} step="0.01" value={refundMinor === null ? '' : refundMinor / 100} onChange={(e) => setRefundMinor(e.target.value === '' ? null : Math.round(Number(e.target.value) * 100))} /></FormField></> : null}
            <FormField label="Notes" htmlFor="pr-notes" className="sm:col-span-3"><Textarea className="min-h-[38px]" value={notes} onChange={(e) => setNotes(e.target.value)} /></FormField>
          </div>
        </div>
        <DialogFooter>
