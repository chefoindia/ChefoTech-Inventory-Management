    ...
    <Card>
      ...
      <CardContent>
        <FormGrid className="sm:grid-cols-3">
          <FormField info="The Gemini model used for chat, reports and reading invoices. Only models your own key can call are listed." label="Main model" htmlFor="ai-model" hint="Chat, reports, invoice reading."><Select value={form.model} disabled={!canManage} onChange={(e) => setForm({ ...form, model: e.target.value })}>{options.map((m) => <option key={m} value={m}>{m}</option>)}</Select></FormField>
          <FormField info="A cheaper, faster model used for short answers and the What is this explanations, to keep your Google bill down." label="Light model" htmlFor="ai-lite" hint="“What is this?” help and short lookups."><Select value={form.liteModel} disabled={!canManage} onChange={(e) => setForm({ ...form, liteModel: e.target.value })}>{options.map((m) => <option key={m} value={m}>{m}</option>)}</Select></FormField>
          <FormField info="What language the assistant answers in. Auto follows whatever language the question was asked in, including Hinglish." label="Reply language" htmlFor="ai-lang"><Select value={form.language} disabled={!canManage} onChange={(e) => setForm({ ...form, language: e.target.value as AiSettingsDto['language'] })}><option value="auto">Match the user (auto)</option><option value="en">Simple English</option><option value="hi">Hindi</option><option value="hinglish">Hinglish</option></Select></FormField>
          <FormField info="How freely the assistant words its answers. Keep it low, around 0.2, so it sticks to your actual figures." label="Creativity (temperature)" htmlFor="ai-temp" hint="Keep low for factual answers."><Input type="number" min={0} max={1} step={0.1} value={form.temperature} disabled={!canManage} onChange={(e) => setForm({ ...form, temperature: Number(e.target.value) })} /></FormField>
          <FormField info="How long a single answer may be. Higher costs more per answer; 2048 suits normal use." label="Max reply tokens" htmlFor="ai-max"><Input type="number" min={256} max={8192} step={256} value={form.maxOutputTokens} disabled={!canManage} onChange={(e) => setForm({ ...form, maxOutputTokens: Number(e.target.value) })} /></FormField>
          <FormField info="How long to wait for Google before giving up and telling the user to try again." label="Timeout (seconds)" htmlFor="ai-timeout"><Input type="number" min={5} max={120} value={Math.round(form.timeoutMs / 1000)} disabled={!canManage} onChange={(e) => setForm({ ...form, timeoutMs: Number(e.target.value) * 1000 })} /></FormField>
          <FormField info="A ceiling on AI usage per month, so a mistake cannot run up your Google bill. Zero means no limit." label="Monthly token limit" htmlFor="ai-limit" hint="0 = no limit. Requests stop when reached."><Input type="number" min={0} step={10000} value={form.monthlyTokenLimit} disabled={!canManage} onChange={(e) => setForm({ ...form, monthlyTokenLimit: Number(e.target.value) })} /></FormField>
        </FormGrid>
      </CardContent>
