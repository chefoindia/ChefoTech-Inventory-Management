    ...
    <Dialog open={open} onOpenChange={(o) => !pending && onOpenChange(o)}>
      <DialogContent title={field ? `Edit ${field.label}` : `New field for ${ENTITY_LABELS[entity].toLowerCase()}`} size="lg">
        <div className="space-y-4">
          <FormGrid>
            <FormField info="What your staff will see next to the box on the form." label="Label" htmlFor="cf-label" required><Input value={label} onChange={(e) => { setLabel(e.target.value); if (!field) setKey(e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '').replace(/^[^a-z]/, 'f$&').slice(0, 40)); }} autoFocus /></FormField>
            <FormField info="The short internal name used in exports and imports. Letters, numbers and underscores only, and it cannot be changed later." label="Key" htmlFor="cf-key" required hint="Stored name; cannot change later."><Input value={key} onChange={(e) => setKey(e.target.value)} disabled={!!field} className="font-mono" /></FormField>
            <FormField info="What kind of value this field holds: text, number, date, a list of choices or a yes/no switch." label="Type" htmlFor="cf-type"><Select value={type} onChange={(e) => setType(e.target.value as CustomFieldType)} disabled={!!field}>{CUSTOM_FIELD_TYPES.map((t) => <option key={t} value={t}>{TYPE_LABELS[t]}</option>)}</Select></FormField>
            <FormField info="Where this field appears among your custom fields. Lower numbers come first." label="Sort order" htmlFor="cf-order"><Input type="number" min={0} value={sortOrder} onChange={(e) => setSortOrder(Number(e.target.value) || 0)} /></FormField>
            {['dropdown', 'multiSelect'].includes(type) ? <FormField info="The choices staff can pick from, one per line. Only used for list fields." label="Options" htmlFor="cf-options" className="sm:col-span-2" hint="One per line. Use label=value to store a different value."><Textarea value={options} onChange={(e) => setOptions(e.target.value)} /></FormField> : null}
            {['number', 'decimal'].includes(type) ? <><FormField info="The smallest number that will be accepted, for number fields." label="Minimum" htmlFor="cf-min"><Input type="number" value={validation.min ?? ''} onChange={(e) => setValidation((v) => ({ ...v, min: e.target.value === '' ? undefined : Number(e.target.value) }))} /></FormField><FormField info="The largest number that will be accepted, for number fields." label="Maximum" htmlFor="cf-max"><Input type="number" value={validation.max ?? ''} onChange={(e) => setValidation((v) => ({ ...v, max: e.target.value === '' ? undefined : Number(e.target.value) }))} /></FormField></> : null}
            {['text', 'longText'].includes(type) ? <><FormField info="The longest text that will be accepted, so notes stay short enough to print." label="Max length" htmlFor="cf-maxlen"><Input type="number" min={1} value={validation.maxLength ?? ''} onChange={(e) => setValidation((v) => ({ ...v, maxLength: e.target.value === '' ? undefined : Number(e.target.value) }))} /></FormField><FormField info="An advanced rule the value must match, for example a fixed code format. Leave blank unless you need it." label="Pattern (regex)" htmlFor="cf-pattern"><Input value={validation.pattern ?? ''} onChange={(e) => setValidation((v) => ({ ...v, pattern: e.target.value || undefined }))} className="font-mono" /></FormField></> : null}
            <FormField info="A line shown under the box to tell staff what to type." label="Help text" htmlFor="cf-help" className="sm:col-span-2"><Input value={helpText} onChange={(e) => setHelpText(e.target.value)} /></FormField>
          </FormGrid>
