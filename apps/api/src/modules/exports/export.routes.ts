...
import { exportEntity, exportOrganization } from './export.service';
import { requirePermission } from '@/middleware/require-permission';

export const exportsRouter = Router();
exportsRouter.use(authenticate, resolveOutlet);

exportsRouter.get('/organization', heavyRateLimit, requirePermission('data.export'), async (req, res) => {
  const result = await exportOrganization(ctxOf(req));
